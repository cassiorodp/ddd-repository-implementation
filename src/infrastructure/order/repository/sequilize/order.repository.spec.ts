...
describe('Order repository test', () => {
  ...
  it('should update an order', async () => {
    ...
    expect(orderModel.total).toBe(80);
  ...
  it('should find an order by id', async () => {
    ...
    expect(foundOrder.items[0].id).toBe('1');
    expect(foundOrder.items[0].name).toBe('Product 1');
    expect(foundOrder.items[0].price).toBe(10);
    expect(foundOrder.items[0].quantity).toBe(2);
    expect(foundOrder.total()).toBe(20);
  });
